...
Configuration file
===================
In the following snippet you can find an example of a `configuration file <https://docs.python.org/3/library/configparser.html>`_. There are four mandatory sections: bed, sample, baseline, and output. Each of these sections has its own mandatory keys and values: bedfile, bamfile, bamfiles, and path, respectively. Basically, these aforementioned values represent the names of the files that will be used in the CNV detection process. Note that VFC root filename(s) must match with the BAM files::
